...
var baseType = function(types, type) {
  var typeDef = (u.typeOf(type) === 'string' ? types[type] : type);
  if (!typeDef) return type;
  var typeSpec = spec.parse((typeDef.type || 'any'), {types: types}),
      typeName = typeSpec && typeSpec.typeNames[0],
      superType = types[typeName];
  if (superType) {
    return baseType(types, superType);
  } else {
    return typeName;
  }
};
...
var isStructure = function(args) {
  ...
  if (u.typeOf(args.structure) === 'object') {
    ...
    if (args.allowMore === false && u.typeOf(args.value) === 'object' &&
      Object.keys(args.value).some(function(k) { return keys.indexOf(k) === -1; })) {
      return false;
    }
    return true;
