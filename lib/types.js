...
var metaTypes = {
  ...
  type: {
    type: 'object',
    validate: function(v) {
      if (v.structure && !(spec.containsType(v.type, 'object') || spec.containsType(v.type, 'array'))) {
        return false;
...
var isType = function(args) {
  ...
  if (typeDef) {
    return (typeDef.type == null || isType({types: args.types, type: typeDef.type, value: args.value})) &&
      ...
      (typeDef.structure == null || isStructure({types: args.types, structure: typeDef.structure, value: args.value})) &&
...
var isStructure = function(args) {
  ...
  if (u.typeOf(args.structure) === 'object') {
    ...
    if (u.typeOf(args.value) === 'array') {
      if (args.structure.length > 1 && args.structure.length !== args.value.length) {
        return false;
      }
      for (var i = 0; i < args.value.length; ++i) {
        var v = args.value[i],
            s = (args.structure.length > 1 ? args.structure[i] : args.structure[0]);
