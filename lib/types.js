  ...
  if (u.typeOf(args.structure) === 'object') {
    ...
    if (u.typeOf(args.value) === 'array') {
      for (var i = 0; i < args.value.length; ++i) {
        var v = args.value[i];
        if (u.typeOf(args.structure[i]) === 'string') {
          if (!isType({types: args.types, type: args.structure[i], value: v})) {
            return false;
          }
        } else {
          if (!isStructure({types: args.types, structure: args.structure[i], value: v})) {
            return false;
          }
        }
      }
