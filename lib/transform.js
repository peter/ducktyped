...
var _mapStructure = function(path, structure, fn) {
  if (u.typeOf(structure) === 'object') {
    ...
    if (structure.length === 1) {
      var value = structure[0],
          newPath = path.concat([0]);
      if (u.isStructure(value)) {
        return [_mapStructure(newPath, value, fn)];
      } else {
        return fn(path, structure);
      }
    } else {
      var result = [];
...
var transformStructure = function(args) {
  ...
  return mapStructure(args.structure, function(path, node) {
    var value = u.getPath(path, args.value);
    if (u.typeOf(node) === 'array' && node.length === 1 && u.typeOf(value) === 'array') {
      return value.map(function(v) {
        return transformType({baseTransform: args.baseTransform, types: args.types, typeSpec: node[0], value: v});
      });
    } else {
      return transformType({baseTransform: args.baseTransform, types: args.types, typeSpec: node, value: value});
    }
  });
};
